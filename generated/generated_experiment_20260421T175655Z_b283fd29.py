...
class Experiment:
    ...
    def run(self) -> Dict[str, Any]:
        try:
            ...
            self._phase_configure_stimulation()
    ...
    def _phase_configure_stimulation(self) -> None:
        logger.info("Phase: configuring stimulation parameters for all electrodes")

        amplitude_ua = min(abs(self.amplitude_ua), 4.0)
        duration_us = min(abs(self.duration_us), 400.0)

        stim_params = []
        for i, electrode_idx in enumerate(self.electrodes):
            stim = StimParam()
        ...
        logger.info(
            "Configured %d electrodes: amplitude=%.1f uA, duration=%.1f us",
            len(stim_params), amplitude_ua, duration_us,
        )

    def _phase_continuous_stimulation(self) -> None:
        logger.info(
            "Phase: continuous stimulation — %d stimulations per electrode, %d electrodes",
        ...
        for electrode_idx in self.electrodes:
            logger.info(
                "Stimulating electrode %d for %d stimulations",
                electrode_idx, self.num_stimulations_per_electrode,
            )
            ...
            for stim_idx in range(self.num_stimulations_per_electrode):
                ...
                self._stimulation_log.append(StimulationRecord(
                    ...
                    extra={"stim_index": stim_idx},
                ))

            logger.info(
                "Completed %d stimulations on electrode %d",
                self.num_stimulations_per_electrode, electrode_idx,
            )

        logger.info(
            "Continuous stimulation phase complete. Total stimulations: %d",
    ...
    def _compile_results(
        ...
        for record in self._stimulation_log:
            stims_per_electrode[record.electrode_idx] = (
                stims_per_electrode.get(record.electrode_idx, 0) + 1
            )
