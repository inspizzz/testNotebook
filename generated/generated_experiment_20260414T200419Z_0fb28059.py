...
class Experiment:
    def __init__(
        ...
        if electrodes is not None:
            ...
            self.electrodes_to_stimulate = [0, 1, 5, 6, 9, 14, 18, 22]
    ...
    def _phase_configure_stimulation(self) -> None:
        logger.info("Phase: configuring stimulation parameters for %d electrodes", len(self.electrodes_to_stimulate))
        ...
        for i, electrode_idx in enumerate(self.electrodes_to_stimulate):
            ...
            sp.phase_amplitude1 = a1
            sp.phase_duration1 = d1
            sp.phase_amplitude2 = a2
            sp.phase_duration2 = d2
        ...
        logger.info("Stimulation parameters configured for electrodes: %s", self.electrodes_to_stimulate)

        self._stim_configs = []
        for i, electrode_idx in enumerate(self.electrodes_to_stimulate):
            trigger_key = i % 16
            self._stim_configs.append({
    ...
    def _phase_continuous_stimulation(self) -> None:
        logger.info(
            "Phase: continuous stimulation — %d stimulations per electrode, %d electrodes, no inter-stim delay",
            self.num_stimulations_per_electrode,
            len(self.electrodes_to_stimulate),
        )

        total_rounds = self.num_stimulations_per_electrode
        num_electrodes = len(self.electrodes_to_stimulate)

        for stim_idx in range(total_rounds):
            for cfg in self._stim_configs:
                electrode_idx = cfg["electrode_idx"]
                trigger_key = cfg["trigger_key"]
                amplitude_ua = cfg["amplitude_ua"]
            ...
            if stim_idx % 100 == 0:
                logger.info(
                    "Stimulation round %d/%d complete, total delivered: %d",
                    stim_idx + 1,
                    total_rounds,
