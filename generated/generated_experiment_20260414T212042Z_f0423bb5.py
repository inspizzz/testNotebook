...
class Experiment:
    def __init__(
        ...
        self._frequency_results: List[FrequencyBlockResult] = []

        assert abs(self.amplitude_ua * self.duration_us - self.amplitude_ua * self.duration_us) < 1e-9, \
            "Charge balance check placeholder"
        assert self.amplitude_ua <= 4.0, "Amplitude exceeds maximum 4.0 uA"
        assert self.duration_us <= 400.0, "Duration exceeds maximum 400 us"
    ...
    def run(self) -> Dict[str, Any]:
        try:
            ...
            for freq_idx, freq_hz in enumerate(self.frequencies_hz):
                logger.info("Starting frequency block %d/%d: %.2f Hz",
                            freq_idx + 1, len(self.frequencies_hz), freq_hz)
                block_result = self._run_frequency_block(freq_hz)
                self._frequency_results.append(block_result)
                logger.info(
                    "Frequency %.2f Hz: response_prob=%.3f, mean_latency=%.2f ms, spike_count=%d",
                    freq_hz,
    ...
    def _configure_stimulation(self) -> None:
        stim = self._build_stim_param(self.stim_electrode, self.amplitude_ua, self.duration_us,
                                       StimPolarity.PositiveFirst, trigger_key=0)
        self.intan.send_stimparam([stim])
        logger.info("Stimulation parameters configured for electrode %d", self.stim_electrode)
    ...
    def _run_frequency_block(self, freq_hz: float) -> FrequencyBlockResult:
        ...
        for trial_idx in range(self.num_trials):
            ...
            try:
                ...
                if not spike_df.empty:
                    ...
                    if time_col is not None and channel_col is not None:
                        resp_spikes = spike_df[spike_df[channel_col] == self.record_electrode]
                        if not resp_spikes.empty:
                            ...
                            latencies = [(st - stim_time_aware).total_seconds() * 1000.0
                                         for st in spike_times
                                         if 0 < (st - stim_time_aware).total_seconds() * 1000.0 <= self.response_window_ms]
                            if latencies:
                                responding_trials += 1
                                total_spike_count += len(latencies)
                                latencies_ms.extend(latencies)
            except Exception as exc:
                logger.warning("Error querying spikes for trial %d at %.2f Hz: %s",
                               trial_idx, freq_hz, exc)
    ...
    def _save_all(self, recording_start: datetime, recording_stop: datetime) -> None:
        ...
        for fr in self._frequency_results:
            freq_results_serializable.append({
                "frequency_hz": fr.frequency_hz,
                "num_trials": fr.num_trials,
                "responding_trials": fr.responding_trials,
                "response_probability": fr.response_probability,
                "mean_latency_ms": fr.mean_latency_ms,
                "total_spike_count": fr.total_spike_count,
            })
        ...
        summary = {
            ...
            "amplitude_ua": self.amplitude_ua,
            "duration_us": self.duration_us,
            "num_trials_per_block": self.num_trials,
            "frequencies_hz": self.frequencies_hz,
            "inter_block_wait_s": self.inter_block_wait_s,
