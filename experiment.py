...
class NeuroStimulationExperiment:
    ...
    def __init__(self, testing_mode=False):
        ...
        if self.testing_mode:
            print("*** TESTING MODE ENABLED - No actual stimulations will be sent ***")
    ...
    def _create_stimulation_parameters(self, electrode_indices, trigger_keys, amplitudes, durations):
        ...
        for i, (electrode_idx, trigger_key, amplitude, duration) in enumerate(
            ...
            if self.testing_mode:
                print(f"[TEST MODE] Would create StimParam {i+1}: Electrode {electrode_idx}, "
                      f"Trigger {trigger_key}, Amplitude {amplitude}µA, Duration {duration}µs")
            else:
    ...
    def _setup_connections(self):
        ...
        if self.testing_mode:
            print("[TEST MODE] Would connect to Intan software and trigger generator")
        else:
    ...
    def _send_parameters(self):
        ...
        if self.testing_mode:
            print("[TEST MODE] Would send stimulation parameters (normally takes 10 seconds)")
        else:
    ...
    def _run_stimulation_sequence(self, experiment_duration_minutes=7, 
        ...
        for iteration in range(total_iterations):
            if self.testing_mode:
                print(f"[TEST MODE] Would send stimulation {iteration + 1}/{total_iterations} at "
    ...
    def _cleanup(self):
        ...
        if self.testing_mode:
            print("[TEST MODE] Would disable all stimulation parameters and close connections")
        else:
    ...
    def run(self, token, electrode_indices=None, amplitudes=None, durations=None,
        ...
        try:
            if self.testing_mode:
                print("[TEST MODE] Would initialize and start experiment")
                print(f"[TEST MODE] Would use electrodes: {electrode_indices}")
            else:
